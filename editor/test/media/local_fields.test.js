...
let newClassEn = 'green'
let newClassEs = 'verde'
...
staticServingPathIntercept.callbackGet = (request) => {
  ...
  const servingUrl = podPathToImg[podPath] || ''
  return {
    'pod_path': podPath,
...
describe('media field', () => {
  ...
  it('should accept input', async () => {
    ...
    // Expand the sub group.
    const groupLabel = await page.$('.selective__field__type__group label')
    await groupLabel.click()
    await page.waitForSelector('.selective__field__type__text[data-field-full-key="class"] input')

    // Change the class.
    await page.click('.selective__field__type__text[data-field-full-key="class"] input', {clickCount: 3})
    await page.keyboard.press('Backspace')
    await page.keyboard.type(newClassEn)
